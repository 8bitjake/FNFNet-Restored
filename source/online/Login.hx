...
class Login extends MusicBeatState
{
		...
		if (FlxG.keys.justPressed.ENTER)
		{
			...
			switch (curselected)
			{
				case 0:
					login();
				case 1:
					FlxG.openURL("http://" + Config.data.addr + "/register.html");
			}
		}
	...
	{
		control = false;
		var request = new Http("https://" + Config.data.addr + "/login");
		request.setPostData('{"username":"' + userbar.text + '", "password":"' + passbar.text + '", "action": "accinfo", "value": 0}');
		...
		request.onData = function(data:String)
		{
			...
			FlxG.save.flush();
			FlxG.switchState(new Account(accdata.id));
		}
        request.onError = function(msg:String){
            trace(msg);
            control = true;
            errtext.text = Json.parse(msg).message;
        }
        request.onStatus = function(code:Int){
            trace("Status " + code);
        }
