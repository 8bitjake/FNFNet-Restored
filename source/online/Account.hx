...
class Account extends MusicBeatState
{
	...
	override function create()
	{
		...
		add(bg);
		var request = new Http("https://" + Config.data.addr + (id == 0 ? "/login/" : "/info/"));
		request.addHeader("Content-Type", "application/json");
		var shit = (id == 0 ? '{"username":"${FlxG.save.data.username}", "password":"${FlxG.save.data.password}"' : '{"id": $id');
		request.setPostData(shit + ', "action": "accinfo", "value": 0}');
		...
		{
			var errore = new Alphabet(0, 0,"Could not connect to server", true);
			errore.screenCenter(XY);
			add(errore);
		};
	...
	override function update(elapsed:Float)
	{
		...
		if (FlxG.keys.justPressed.R)
			FlxG.openURL("http://" + Config.data.addr + "/edit.html");
		if (FlxG.keys.justPressed.L)
		{
			FlxG.switchState(new Leaderboard());
		}
		if (false)//(FlxG.keys.justPressed.S)
		{
