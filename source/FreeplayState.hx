...
class FreeplayState extends MusicBeatSubstate
{
	...
	override function create()
	{
		...
		scoreBG = new FlxSprite(scoreText.x - 6, 0).makeGraphic(Std.int(FlxG.width / 3.25), 105, 0xFF000000);
		scoreBG.alpha = 0.6;
		...
		var mdl = new haxe.Http("https://" + Config.data.resourceaddr + "/mods.json");
	...
	override function update(elapsed:Float)
	{
		...
		if (accepted && !disablecontrol)
		{
			...
			sys.thread.Thread.create(function()
			{
				...
				var http = new haxe.Http('https://' + Config.data.resourceaddr + '/songs/' + songs[curSelected].songName.toLowerCase() + '/chart$modif.json');
				...
				http.onData = function(data:String)
				{
					...
					PlayState.modinst.addEventListener(Event.COMPLETE, function(e:Event)
					{
						...
						PlayState.modvoices.load(new URLRequest('https://' + Config.data.resourceaddr + '/songs/'
							+ songs[curSelected].songName.toLowerCase() + '/Voices.ogg'));
					});
					PlayState.modinst.load(new URLRequest('https://' + Config.data.resourceaddr + '/songs/'
						+ songs[curSelected].songName.toLowerCase() + '/Inst.ogg'));
				}
