...
{
	public static var songname = "titular";
	...
	override function create()
	{
		...
		gf = new test.CharacterTest(0, 0, "gf");
		gf.scrollFactor.set(0.95, 0.95);
		dad = new test.CharacterTest(0, 0, "dad", false);

		boyfriend = new BFTest(0, 0, "bf");
	...
	override function update(elapsed:Float)
	{
		...
		if (FlxG.keys.pressed.UP)
			camFollow.y -= FlxG.keys.pressed.SHIFT ? 5 : 1;
		if (FlxG.keys.pressed.DOWN)
			camFollow.y += FlxG.keys.pressed.SHIFT ? 5 : 1;
		if (FlxG.keys.pressed.LEFT)
			camFollow.x -= FlxG.keys.pressed.SHIFT ? 5 : 1;
