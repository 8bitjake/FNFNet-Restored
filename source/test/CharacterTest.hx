...
class CharacterTest extends FlxSprite
{
	...
	public var camPos:{x:Int, y:Int} = {x: 0, y: 0};
	...
	public function new(x:Float, y:Float, ?character:String = "bf", ?isPlayer:Bool = false, ?shit:FlxAtlasFrames)
	{
		...
		antialiasing = true;
		if (FileSystem.exists("test/" + characterdecide + ".json"))
		{
			var chardata = haxe.Json.parse(File.getContent("test/" + characterdecide + ".json"));
			if (Std.is(chardata.character.position.y, Float) && Std.is(chardata.character.position.x, Float))
			{
				x = chardata.character.position.x;
				y = chardata.character.position.y;
			}
			loaded = false;
			BitmapData.loadFromFile("test/" + characterdecide + ".png").then(function(image)
			{
				frames = FlxAtlasFrames.fromSparrow(image, File.getContent("test/" + characterdecide + ".xml"));
				if (character != "gf")
				{
				...
				if (chardata.character.size.width != 0)
